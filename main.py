...
ROOT_FOLDER = os.getenv("ROOT_FOLDER", "items")  # Updated to 'items'
...
async def get_item_image(
    ...
    """
    Fetch PNG image by ID from dynamic batch folders under `items/`.
    """
    logger.info(f"Received request for image ID: {id}")
    if key != API_KEY:
        logger.error(f"Invalid API key provided: {key}")
...
async def list_images(
    ...
    """
    Scan all batch folders under `items/` and list all PNG files.
    """
